...
Connection.prototype.readResponseFrame = function (buffer, callback) {
  ...
  response.flags = reader.buffer.readUInt8(1);
  response.stream = reader.buffer.readUInt8(2);
  response.opcode = reader.buffer.readUInt8(3);
  ...
  if (bodyType) { response.body = bodyType.read(reader); }
  callback(response);
};
